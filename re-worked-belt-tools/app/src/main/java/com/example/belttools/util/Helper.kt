...
fun addRoomSquaresOrBoxes(previousResults: MutableList<String>, newResult: String): List<String>? =
...
fun sumListOfString(btnSquares: List<String>): Double {
    ...
    btnSquares.forEach { squaresSummed += it.toDouble() }
...
fun decimalToFraction(num: Double): String {
    if (num < 0) return "-" + decimalToFraction(-num)
    val tolerance = 1.0E-6
    var h1 = 1.0; var h2 = 0.0; var k1 = 0.0; var k2 = 1.0
    var b = num
    do {
...
fun doesSKUContainId(list: List<SKU>, id: Long): Boolean {
    list.forEach { if (it.id == id) return true }
    return false
}
...
fun getDepartmentNames(departments: List<Department>): MutableList<String> {
    ...
    departments.forEach { if (it.notes != null) departmentNames.add(it.name)  }
...
fun filterExtensionNotNull(departments: List<Department>): List<Department> {
    ...
    departments.forEach { if (it.extensions != null) newDepartments.add(it) }
